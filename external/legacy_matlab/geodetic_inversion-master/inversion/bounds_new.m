function [lb,ub]=bounds_new(NS,NT,tSm,add_col,Con,slip_model)
...
Npatch = sum(tSm);
% lb=-5e1*ones(NT*Npatch,1);  %lower bound in cm
% ub= 5e1*ones(NT*Npatch,1);  %upper bound
% lb=-1.2e3*ones(NT*Npatch,1);  %lower bound in cm
% ub= 1.2e3*ones(NT*Npatch,1);  %upper bound

% 
lb=-5.5e2*ones(NT*Npatch,1);  %lower bound in cm, default for Tingri mainshock, default 5.5e2
ub= 5.5e2*ones(NT*Npatch,1);  %upper bound
lb(Npatch+1:2*Npatch) = -10e2;   % dominated by dip slip, default for Tingri mainshock, default 6e2
ub(Npatch+1:2*Npatch) = 10e2;

% lb=-1.5e2*ones(NT*Npatch,1);  %lower bound in cm for Tingri postseismic, default 1.5e-4
% ub= 1.5e2*ones(NT*Npatch,1);  %upper bound
% lb(Npatch+1:2*Npatch) = -1.5e2;   % dominated by dip slip for Tingri postseismic, default 1.2e3
% ub(Npatch+1:2*Npatch) = 1.5e2;





Con2=[Con(1),-Con(2),Con(3)]; % reverse the positivity constrain for subfault, default for Tingri mainshock
...
%% Higher top displacement (test)

% indx_top=find(patch_layer>=2);
% lb(indx_top) = -4e2;
% lb(indx_top+Npatch) = -1e2;
% ub(indx_top) = 4e2;
% ub(indx_top+Npatch) = 1e2;
...
for i=1:NS1
 ...
 k2=sum(tSm(1:i+1));
 if i>1
     Con=Con2;% Reverse the constrain for the subfault
     % Con=[0 0 0];%??
 end
 for k=k1:k2
