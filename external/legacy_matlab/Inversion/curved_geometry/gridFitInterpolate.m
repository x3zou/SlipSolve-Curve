function [xb, yb, zb, xp, yp, zp] = gridFitInterpolate(xfit, yfit, zfit, xin, yin,smoothness)
    ...
    zb = gridfit(xfit, yfit, zfit, xin, yin,'smoothness',smoothness);
    [xb, yb] = meshgrid(xin, yin);
    
    xp = movmean(xin, 2, 'Endpoints', 'discard');
    yp = movmean(yin, 2, 'Endpoints', 'discard');
    zp = gridfit(xfit, yfit, zfit, xp, yp,'smoothness',smoothness);
