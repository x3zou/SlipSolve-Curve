function [Ge,Gn,Gp]=calc_green_gps_3d_edcmp_xyz(slip_model,data_insar,finp)
% Calculate the Green's function for InSAR observations using EDGRN
% Instead of projecting it to sattlite deformation, keep the xyz
...
for ipatch = 1:npatch;
   ...
   %slip_patch=1;
   slip_patch=slip_model(ipatch,11); %scale by the moment? tested by Xiaoyu
   rake_patch=0;
...
for ipatch = 1:npatch;
   ...
   dip_patch=slip_model(ipatch,10);
   %slip_patch=1;
   slip_patch=slip_model(ipatch,11); %scale by the moment (area)? tested by Xiaoyu
   rake_patch=90;
   [pxs,pys,pzs,pmoment]=edcdisc(xs_patch,ys_patch,zs_patch,slip_patch,lp_patch,...
    wp_patch,strike_patch,dip_patch,rake_patch,nz,z1,z2,dr,dz);
    nrec=length(xrec); %number of observation points
