...
SlipSolve-curve uses two distribution layers:

- **Normal Git repository:** MATLAB source, workflows, configuration,
  documentation, triangular geometry, seven sampled inversion tracks, the
  velocity model, the independent sampled observations, data manifests, and
  checksums.
...
1. Choose a project license and verify that each redistributed legacy helper
   may legally be included. Preserve all third-party notices.
2. Keep `.gitignore` unchanged. It excludes generated figures, inversion
   results, downloaded `.grd` files, Green products, AppleDouble files, and all
   products above GitHub's normal file limit. Never add them with `git add -f`.

All workflow MATLAB dependencies are already internalized under
`external/legacy_matlab`. Their paths, sizes, hashes, source notices, and
citation guidance are documented in `external/README.md` and the dependency
manifest stored beside the copied files.

### Upload The Example Data To Google Drive

The prepared upload folder is named:

```text
SlipSolve-curve-example-data/
```

Upload that complete folder to Google Drive without renaming or flattening
its contents. It contains this install-ready structure:
...
SlipSolve-curve-example-data/
  ...
  greens/
    ...
    composite_full_xyz/G_u.mat
    composite_sampled_tracks/sampled_composite_greens.mat
```

After uploading, enable link sharing for download, replace
`GOOGLE_DRIVE_DOWNLOAD_LINK` in this README with the public URL, and test the
download using an empty clone. Users install the data by copying the bundle's
`data` and `greens` folders into the repository root.

### Create And Push The Repository

Create an empty GitHub repository named `SlipSolve-curve`. Do not initialize it
with a README, `.gitignore`, or license because those should come from this
folder. Then, from the SlipSolve-curve project root, run:
...
git add .
git status --short
git commit -m "Initial SlipSolve-curve release"
git remote add origin https://github.com/YOUR_GITHUB_USER/SlipSolve-curve.git
git push -u origin main
```

Inspect `git status` before committing. No file under `greens`, `inversions`,
or `figures` should appear except explicitly retained `.gitkeep` files. The
downloaded `.grd` files under `data/raw` must also remain absent from the Git
staging list.
